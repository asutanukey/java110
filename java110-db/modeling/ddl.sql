...
ALTER TABLE `cls_room`
	ADD CONSTRAINT `PK_cls_room` -- 강의실 기본키
		PRIMARY KEY (
			`crno` -- 강의실번호
		);
...
-- 강의실사진
ALTER TABLE `cr_phot`
	ADD CONSTRAINT `PK_cr_phot` -- 강의실사진 기본키
		PRIMARY KEY (
			`cpno` -- 강의실사진번호
		);
...
CREATE TABLE `p2_subj` (
	...
	`COL7` VARCHAR(255) NOT NULL COMMENT '강의과목' -- 강의과목
)
COMMENT '과목';
...
-- 강의
ALTER TABLE `lect`
	ADD CONSTRAINT `FK_cls_room_TO_lect` -- 강의실 -> 강의
		FOREIGN KEY (
			`crno` -- 강의실번호
		)
		REFERENCES `cls_room` ( -- 강의실
			`crno` -- 강의실번호
		);
...
-- 매니저
ALTER TABLE `mgr`
	ADD CONSTRAINT `FK_p2_memb_TO_mgr` -- 회원 -> 매니저
		FOREIGN KEY (
			`mno` -- 회원번호
		)
		REFERENCES `p2_memb` ( -- 회원
...
-- 강의실사진
ALTER TABLE `cr_phot`
	ADD CONSTRAINT `FK_cls_room_TO_cr_phot` -- 강의실 -> 강의실사진
		FOREIGN KEY (
			`crno` -- 강의실번호
		)
		REFERENCES `cls_room` ( -- 강의실
...
-- 강의배정
ALTER TABLE `p2_tchr_lect`
	ADD CONSTRAINT `FK_lect_TO_p2_tchr_lect` -- 강의 -> 강의배정
		FOREIGN KEY (
			`lno` -- 강의번호
		)
		REFERENCES `lect` ( -- 강의
